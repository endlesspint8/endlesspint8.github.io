...
<p>At the start of this process I wasn't sure how well the Brier Score (BS) measure would resonate, with myself included. I've done my best to bring it up, occasionally highlighting it, but also attempting not to beat the reader over the head with it. <img src="http://endlesspint.com/gallery/2016/football-picks/wk14_slopegraph.png" alt="wk14_slopegraph" width="360" height="900" align="right"> In some respects the weekly visuals are in effort to make BS a more intuitive yardstick. A couple of weeks back I thought I may have hit upon an approach that drove home the point without even specify the exact metric. This was back in week 10 where I provided a throwaway graph in the form of a bubble chart, plotting the confidence percentage by source against the actual winning percentages. What that chart meant to show in a very simple, though busy and cluttered way, what is the general reliability of the sources or lack there of. Simply by identifying the color of the bubble (source), it's size (number of said predictions), and placement above or below the 45° line (beating or missing expectations, respectively) the user could see who was the most reliably trustworthy.</p>

<p>This week I apply the same motivation with updated data to present this take away once more. Instead of dots or bubbles are used lines in a slope graph. The left hand position of the plot represents the percentage confidence of the prognosticating sources. On the opposite side, the right side, we have the resulting winning percentages. Line colors represent the sources, Line thickness a stand-in for game counts, and the general slope of the line, whether upwards or down, represents how far off the sources are from their intended outcome. A flat horizontal line would signify matched expectations and results. At upward moving line from left to right would indicate beaten expectations while a downward slanting line the opposite.</p>

<p>A few things of note. The human panel sources have a few were lines, thicker lines, and unfortunately downward lines. The fewer and thicker lines go hand-in-hand as a human panels have a set number of experts reliably resulting in the same conference percentages week to week. The data models being more "fine-tuned" provide something closer to a continuous range of confidence percentages those resulting in many more lines at thinner ones. While it is not possible for us to break out The human panel pigs into more find percentages, which would perhaps allow us to see some up or moving lines, we are able to bucket The data model pics into fewer ranges. Perhaps by doing so we can more reliably compare the computers for us to humans and see how well they perform against one another. </p>
