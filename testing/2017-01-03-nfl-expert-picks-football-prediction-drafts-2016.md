...
<p>A few things of note. The human panel sources have a fewer, thicker, downward sloping lines. The fewer and thicker aspect go hand-in-hand as the human panels have a set number of experts, reliably resulting in repetitive confidence percentages week-to-week. The data models being more "fine-tuned" provide something closer to a continuous range of confidence percentages, resulting in many more and thinner lines. While it is not possible for us to break out the human panel picks into more fine-levle percentages, which might allow us to see some upward moving lines, we are able to (bucket the data model picks)[]. Grouping the data model confidence percentages will allow us to more reliably compare the computers versus humans and see how well they perform against one another.</p>
